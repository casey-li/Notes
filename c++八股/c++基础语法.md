...
1. **参数入栈**：在函数调用前，调用者函数会将被调函数所需的参数按照与被调函数的形参顺序相反的顺序压入栈中。即，从右向左依次将被调函数所需的参数压入栈。

2. **返回地址入栈**：调用者函数使用 call 指令调用被调函数，call 指令会将当前 call 指令的下一条指令的地址（即函数调用后要执行的下一条指令地址）压入栈中，作为被调函数的返回地址。这个压栈操作隐含在 call 指令中。

3. **保存现场**：在被调函数中，为了保护当前调用者函数的现场（即栈帧, 例如调用者函数的栈底地址和栈顶地址, 返回值等信息），被调函数会先保存调用者函数的栈底地址，即 push ebp，然后再保存调用者函数的栈顶地址，即将当前被调函数的栈顶地址赋值给 ebp 寄存器（mov ebp, esp）。这样，被调函数就保存了调用者函数的栈底地址和栈顶地址。
...
栈的增长方向是向低地址方向，因此，**参数和返回地址都会先入栈，然后是调用者函数的栈帧（保存现场），最后是被调函数的局部变量和临时变量**。函数调用结束后，栈会根据栈帧信息恢复现场，从而返回到调用者函数继续执行后续指令
